...
class Grid:
    ...
    def random(self):
        ...
        if not self.subgrids:  # If there are no existing subgrids.
            x = self.x1 + round(random.random() * (self.x2 - self.x1),2)  # Generate a random x-coordinate.
            y = self.y1 + int(random.random() * (self.y2 - self.y1))  # Generate a random y-coordinate.
            four = [
                (self.x1, x, self.y1, y),
                (x, self.x2, self.y1, y),
                (self.x1, x, y, self.y2),
            ...
            for a, b, c, d in four:
                if a != b and c != d:
                    ...
                    self.subgrids.append(subgrid)  # Add the subgrid to the list of subgrids.
        else:  # If subgrids already exist.
            random.choice(self.subgrids).random()  # Choose a random subgrid and generate a random subgrid within it.
    ...
    def flatten(self):
        ...
        for subgrid in self.subgrids:  # For each subgrid instance.
            if not subgrid.subgrids:  # If the subgrid has no subgrids of its own.
                result.append((subgrid.x1, subgrid.x2, subgrid.y1, subgrid.y2))  # Append its coordinates to the result list.
            else:  # If the subgrid has its own subgrids.
                result.extend(subgrid.flatten())  # Recursively flatten its subgrids and extend the result list.

        return result  # Return the flattened subgrid coordinates.


def random_scenario(number_of_nodes: int, t_start: float, t_end: float, approx_order_of_communities: int) -> list[list]:
    """
